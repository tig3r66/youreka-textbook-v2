...
server <- function(input, output, session) {
  ...
  output$CLT.descr = renderText({
    ...
    paste0("According to the Central Limit Theorem, given sufficiently large random samples
           from a population with replacement, the distribution of the sample means will be approximately
           normally distributed. The mean of the sampling distribution should be approximately equal to
           the population mean (μ=", m_pop, ") and the standard error (the standard deviation of sample means)
           should be approximately equal to the SD of the population divided by square root of sample size
           (stderr=", s_pop, "/sqrt(",n, ") = ", se,"). Below is our sampling distribution graph. To help
           compare, population distribution plot is also displayed on the right.")
  })
}
