...
ggplot(dem07, aes(as.factor(X2007))) +
  geom_bar()  # this is NOT a histogram
```
...
```{r}
nrow(dem07[dem07$X2007 >= -2 & dem07$X2007 <= 5,])
dem07 %>% filter(X2007 >= -2 & X2007 <= 5) %>% nrow()
```
...
To run an ANOVA, we need to check if
  ...
  1.  The variance is is equal for *each group*.
  2. The data distributes normally within *each group*, and
...
In the test with `*`, there is one more term `Income_Level_2007:demLev`. This is the interaction between the two variables. In this case, the interaction of of the two variables also contribute significantly to the difference in `lifeExp`.
...
```{r}
ggplot(data = dat.2007, aes(x = log(gdpPercap), y = lifeExp)) +
  geom_point()
```
...
The p-value suggests the correlation is significant. The coefficient of 0.8 suggests a positive correation (y increases as x increases). In case you see a negative value, the correlation if negative (one variable going up while the other going down).
...
The linear equation is: lifeExp = 7.19 $\cdot$ log_GDP + 4.7738.
...
```{r}
cor.test(~ lifeExp + log_GDP, data = dat.2007, method = 'spearman')
```
