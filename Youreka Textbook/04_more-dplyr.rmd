...
When I first cleaned this Gapminder excerpt, I was a [`camelCase`](http://en.wikipedia.org/wiki/CamelCase) person, but now I'm all about [`snake_case`](http://en.wikipedia.org/wiki/Snake_case). Let's rename some variables!
...
names(sexes)
# now look
sexes <- factor(c('female', 'male', 'female'))
names(sexes)
```
...
`ggplot2` can generate many types of graphs. A great summary can be found on the R studio [ggplot2 cheat sheet](https://rstudio.com/wp-content/uploads/2015/03/ggplot2-cheatsheet.pdf).
...
Now this data is interesting. Take a look at the life expectancy in Rwanda from 1987 to 1992.
...
gapminder %>%
  select(country, year, lifeExp) %>%
  filter(year == 1992, country == 'Rwanda')
```

Ponder this data for a while. Think about the real-life implications and the code. What you're seeing is genocide in dry statistics on average life expectancy.
